...
Notes: 
-Are baseline level droplets recognized with grad?
-Split 7p events in grad?
"""
import numpy as np
...
args = parser.parse_args()#['midpoint', 'rawsort_cont1_2h.txt', '-o', 'peaks-test.txt'])
v = args.verbose
out = args.output
...
def loaddat(filename):
    ...
    #Detect dataformat and rewrite
    if dat.shape[1] == 2:
        xn, yn = zip(*dat)
        xn = np.array(xn)
        if xn[1] == 0.000026:  #Legacy file format
            xn = xn * 1000
        yn = np.array(yn)
    elif dat.shape[1] == 3:
        first, second, third  = zip(*dat)
        answer = input('Wrong dataformat detected. Overwrite? (y/n): ')
        if answer == "y":
            out = open(filename, "w")
        else:
            out = open("rewrite_%s"%filename, "w")
        if second == third:
            xn = np.array(first)
            yn = np.array(second)
            xn = xn * 1000
            ...
            for i in range(len(first)):
                second[i] = second[i] + (r * 263.131579)
                if first[i] == 9999:
                    r  += 1
            for i in range(len(first)):
                out.write("%.6f \t %.6f \n" % (second[i], third[i]))
            xn = np.array(second)
            yn = np.array(third)
            print("Non-contiguous data repaired")
        out.close()            
    else:
        print("error loading data")
        
    #yn = 10 - yn   #Turn upside down: Not anymore!
    return xn, yn
    ...
    for i in range(len(yn - bgnum)):
        ...
        if yn[i] < bgL < yn[i+1]:  #Going up: event complete
            ...
            if c > frac:
                ...
                if 0.2 < size < 4:  #check size appropriate, everything really should be between 0.2 and 4 ms
...
if method != 'rewrite':    
    ...
    #Final scatterplot with jitter
    var = 0.026313     
    for i in range(len(residence)):
        residence[i] = residence[i] + ((random()-0.5)*var)
    plt.figure(figsize=(10,6))
    plt.scatter(residence, peakY, alpha=0.1)
    plt.ylabel("Detection signal [V]")
    plt.xlabel("Residence Time [ms]")
    plt.title("Final scatter plot with jitter")
